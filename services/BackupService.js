...
class BackupService {
    constructor(githubToken, gistId) {
        ...
        this.localStorage = new LocalStorageService_1.LocalStorageService();
    }
    ...
    async loadInitialData() {
        ...
        try {
            ...
            console.log('⚠️ Gist 연결 실패 - 오프라인 모드:', error.message);
            this.isOnline = false;
        }
        // 2. 로컬 데이터 로드
        try {
    ...
    async mergeGistAndLocalData(gistData, localData) {
        try {
            ...
            if (todayLocalIndex >= 0) {
                ...
                allDailyData[todayLocalIndex] = LocalStorageService_1.LocalStorageService.mergeDailyData(allDailyData[todayLocalIndex], gistAsDailyData);
            }
            else {
                // 오늘 날짜 데이터가 없으면 추가
     ...
     * DailyData 배열을 UsageCache로 변환
     */
    convertDailyDataToUsageCache(dailyDataArray) {
